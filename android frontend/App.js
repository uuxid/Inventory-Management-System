...
export default function App() {
  ...
  //<changes>
  // --- Place this inside your App() component with your other state hooks ---
const [permission, requestPermission] = useCameraPermissions();
const [scannerModalVisible, setScannerModalVisible] = useState(false);
const [scanned, setScanned] = useState(false);
const [scanActionModalVisible, setScanActionModalVisible] = useState(false);
const [scanResult, setScanResult] = useState({ sku: '', name: '', currentLocation: '' });
const [scanForm, setScanForm] = useState({
  movementType: 'IN', // 'IN' for Add, 'OUT' for Remove
  quantity: '1',
  location: '',
});
  ...
  //<changes>
  // --- Add this handler function to process the scanned QR/Barcode ---
const handleBarCodeScanned = ({ type, data }) => {
  setScanned(true);
  setScannerModalVisible(false); // Instantly close camera overlay

  if (!data) return;

  // 1. Aggressive cleaning: Remove non-printable characters and spaces
  // Equivalent to Java's .replaceAll("[\\p{C}\\p{Z}]", "")
  const cleanSku = data.trim().replace(/[\p{C}\p{Z}]/gu, '');
  console.log("Scanned Raw:", data);
console.log("Cleaned Payload String:", cleanSku);

  // 2. Look up the product in your local products state array to populate details
  const matchedProduct = products.find(
    (p) => p.sku?.toLowerCase() === cleanSku.toLowerCase() || p.barcode === cleanSku
  );

  setScanResult({
    sku: cleanSku,
    name: matchedProduct ? matchedProduct.name : 'New/Unknown Product',
    currentLocation: matchedProduct ? matchedProduct.location : '',
  });

  // 3. Prep the action form defaults
  setScanForm({
    movementType: 'IN',
    quantity: '1',
    location: matchedProduct ? matchedProduct.location || '' : '',
  });

  // 4. Open the action picker modal
  setScanActionModalVisible(true);
};

const handleScanActionSubmit = async () => {
  const qty = Number(scanForm.quantity);
  if (isNaN(qty) || qty <= 0) {
  ...
  setLoading(true);
  setError('');
  setScanActionModalVisible(false);

  // Compute final value change: Positive for incoming, Negative for outgoing
  const quantityChange = scanForm.movementType === 'IN' ? qty : -qty;

  try {
    // 1. Force your scanned payload string to uppercase to match DB records perfectly
    const targetQrCode = scanResult.sku.toUpperCase();

    // 2. Mirror your ProductRequest DTO requirements exactly
    // Since your backend updateByQr handles processing implicitly, we send the relative count change
    const payload = {
      quantityOnHand: quantityChange,
    };

    // 3. Construct URL targeting the @PutMapping("/barcode/{qr}") mapping
    await request(
      `/api/products/barcode/${targetQrCode}`, 
      {
        method: 'PUT', // Matches your controller mapping type
    ...
    Alert.alert('Stock Updated', `Successfully updated stock for: ${targetQrCode}`);
    
    // Refresh your dashboard list values
    await handleRefresh();
  } catch (apiError) {
    setError(apiError.message || 'Failed to update stock via API');
    Alert.alert('API Error', apiError.message || 'Could not update inventory.');
  } finally {
    setLoading(false);
    setScanned(false); // Reset the scanner engine state
  }
};

// --- Function to trigger permission request and open modal ---
  ...
  return (
    <View style={styles.section}>
      <View style={styles.panel}>
        ...
        {/* Combined Search and Scan Layout Wrapper */}
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.container}>
		...
		{/* Scan Action Confirmation Overlay Form */}
<Modal
  visible={scanActionModalVisible}
  animationType="fade"
  transparent={true}
  onRequestClose={() => {
    setScanActionModalVisible(false);
...
>
  <View style={styles.modalCenteredView}>
    <View style={styles.modalFormContent}>
      <Text style={styles.modalFormTitle}>Inventory Adjustment</Text>
      
      {/* Product Information Context Box */}
      <View style={styles.scannedInfoBox}>
        <Text style={styles.infoLabel}>SKU: <Text style={styles.infoValue}>{scanResult.sku}</Text></Text>
        <Text style={styles.infoLabel}>Product: <Text style={styles.infoValue}>{scanResult.name}</Text></Text>
        {scanResult.currentLocation ? (
          <Text style={styles.infoLabel}>Current Loc: <Text style={styles.infoValue}>{scanResult.currentLocation}</Text></Text>
        ) : null}
      </View>

      {/* Segmented Transaction Selector Control */}
      <Text style={styles.fieldLabel}>Action Type</Text>
      <View style={styles.segmentedControlRow}>
        <TouchableOpacity
          style={[
            styles.segmentButton,
            scanForm.movementType === 'IN' && styles.segmentButtonActiveIn,
          ]}
          onPress={() => setScanForm({ ...scanForm, movementType: 'IN' })}
        >
          <Text style={[styles.segmentButtonText, scanForm.movementType === 'IN' && styles.textWhite]}>
            ➕ Add Stock (In)
          </Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity
          style={[
            styles.segmentButton,
            scanForm.movementType === 'OUT' && styles.segmentButtonActiveOut,
          ]}
          onPress={() => setScanForm({ ...scanForm, movementType: 'OUT' })}
        >
          <Text style={[styles.segmentButtonText, scanForm.movementType === 'OUT' && styles.textWhite]}>
            ➖ Remove Stock (Out)
          </Text>
        </TouchableOpacity>
      </View>
      ...
      <TextInput
        ...
        onChangeText={(text) => setScanForm({ ...scanForm, quantity: text })}
        placeholder="Enter adjustment count"
        placeholderTextColor="#6d7f98"
      />

      {/* Storage Location Input Field */}
      <Text style={styles.fieldLabel}>Warehouse Location / Bin</Text>
      <TextInput
        style={styles.input}
        value={scanForm.location}
        onChangeText={(text) => setScanForm({ ...scanForm, location: text })}
        placeholder="e.g., Aisle 4-Shelf B"
        placeholderTextColor="#6d7f98"
      />

      {/* Interface Call to Action Footers */}
      <View style={[styles.rowGap, { marginTop: 12 }]}>
        <TouchableOpacity
          style={[styles.inlineButton, { flex: 1, backgroundColor: '#6d7f98' }]}
          onPress={() => {
            setScanActionModalVisible(false);
            setScanned(false);
          }}
        ...
        <TouchableOpacity
          style={[styles.inlineButton, { flex: 2, backgroundColor: '#007AFF' }]}
          onPress={handleScanActionSubmit}
        >
          <Text style={styles.inlineButtonText}>Submit Update</Text>
        </TouchableOpacity>
      </View>
    </View>
  </View>
...
>
  <SafeAreaView style={styles.scannerContainer}>
    <View style={styles.scannerHeader}>
      <Text style={styles.scannerTitle}>Scan Item QR / Barcode</Text>
      <TouchableOpacity 
    ...
    <CameraView
      ...
      barcodeScannerSettings={{
        barcodeTypes: ['qr', 'ean13', 'code128', 'upc_a'], // Adjust to match your system specs
      }}
    />
    
    {/* Visual Overlay Target Box */}
    <View style={styles.overlayContainer}>
      <View style={styles.scanTargetBox} />
      <Text style={styles.overlayText}>Center the barcode or QR code inside the box</Text>
    </View>
  </SafeAreaView>
</Modal>
...
const styles = StyleSheet.create({
  ...
  segmentButtonActiveIn: {
    backgroundColor: '#38a169',
    borderColor: '#38a169',
  },
  segmentButtonActiveOut: {
